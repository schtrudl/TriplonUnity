...
public class Trail1 : MonoBehaviour
{
    ...
    private GameObject _previusSegment;
    private Vector3 _previusColliderCenter;
    private Vector3 _previusColliderSize;
    ...
    void LateUpdate()
    {
        ...
        if (_previusSegment)
        {
            BoxCollider boxCollider = _previusSegment.AddComponent<BoxCollider>();

            // Copy properties from _previusCollider to the new BoxCollider
            boxCollider.center = _previusColliderCenter;
            boxCollider.size = _previusColliderSize;
        ...
        // Update all previus variables
        _previusColliderCenter = (min + max) / 2f;
        _previusColliderSize = max - min;
        _previusSegment = segmentObject;
        _previusUpL = _upL.transform.position;
        _previusUpR = _upR.transform.position;
        _previusDownL = _downL.transform.position;
        _previusDownR = _downR.transform.position;
